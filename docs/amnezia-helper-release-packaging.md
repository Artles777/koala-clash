...
The helper Go source now lives in this repository at `native/amnezia-helper/`.
Prebuilt helper binaries are not tracked source files. The build pipeline
generates them under `helper-artifacts/` and stages validated copies under
`extra/files/` for `electron-builder`.

## Generated Artifact Contract

The helper build writes generated artifacts under:
...
The adjacent `.json` manifest is generated by `pnpm build:amnezia-helper`:
...
## Building Helper Artifacts

Build one target from the in-repository Go module:

```bash
pnpm build:amnezia-helper -- --target linux --arch x64
```

Platform aliases are available:

```bash
pnpm build:amnezia-helper:win -- --arch x64
pnpm build:amnezia-helper:mac -- --arch arm64
pnpm build:amnezia-helper:linux -- --arch x64
```

Build the full desktop matrix:

```bash
pnpm build:amnezia-helper -- --all
```

By default the build uses `CGO_ENABLED=0` for portable cross-compilation. Override
with `--cgo-enabled 1` only when the target toolchain is available.

## Preparing Artifacts

Prepare one target. This builds from `native/amnezia-helper` first, then stages
...
An explicit `--source-root` is treated as an external/generated artifact root and
does not build helper source unless `--build` is also passed. Use `--skip-build`
when you want to stage already generated local artifacts without rebuilding:

```bash
pnpm amnezia-helper:prepare -- --target darwin-arm64 --skip-build
```
...
`helper-artifacts/` and `extra/` are ignored by git. `extra/` is consumed by
`electron-builder` through `extraResources`.

## Build-Time Validation

The desktop build scripts run helper preparation before `electron-builder`.
Preparation builds helper source automatically for the requested platform/arch:
...
- the generated helper binary is missing
